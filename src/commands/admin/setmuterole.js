...
module.exports = class SetMuteRoleCommand extends Command {
  ...
  run(message, args) {
    ...
    const embed = new MessageEmbed()
      ...
      .setDescription(`Le Rôle Mute a bien été mise a jours ${success}`)
    ...
    if (args.length === 0) {
      ...
      return message.channel.send(embed.addField('Rôle Mute', `${oldMuteRole} ➔ \`Aucun\``));
    }
    ...
    message.channel.send(embed.addField('Rôle mute', `${oldMuteRole} ➔ ${muteRole}`));
  }
};