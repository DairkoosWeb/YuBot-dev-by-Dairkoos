    ...
    const buildEmbed = (current, embed) => {
      ...
      for (let i = current; i < max; i++) {
        embed // Build warning list
          ...
          .addField('Date', warns.warns[i].date, true);
        amount += 1;
      }

      return embed
        .setTitle('Liste des warns ' + this.client.utils.getRange(warns.warns, current, 5))
        .setDescription(`Montrant \`${amount}\` de ${member} \`${count}\` warns.`);
    };

    if (count == 0) message.channel.send(embed
      .setTitle('Liste des Warns [0]')
